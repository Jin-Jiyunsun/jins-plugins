...
final class SetEffectsData
{
	...
	static
	{
		...
		sets.add(fourPiece("Ahrim's set",
			...
			"Autocasts Ancient Magicks, with a 25% chance of 30% increased spell damage."));
		...
		sets.add(fourPiece("Verac's set",
			"Attacks have a 25% chance to ignore the target's armour, Defence level, and protection prayers.",
		...
		sets.add(new ItemSet("Justiciar armour",
			"Reduces incoming melee/ranged/magic damage (outside PvP) by (your defensive bonus for that style) / 3000, min. 1 damage reduced.",
			threePiece(ItemID.JUSTICIAR_FACEGUARD, ItemID.JUSTICIAR_CHESTGUARD, ItemID.JUSTICIAR_LEG_GUARDS)));

		sets.add(new ItemSet("Inquisitor's armour",
			"Adds crush accuracy and damage while using the crush attack style: great helm +0.5%, hauberk +1%, plateskirt +1% (up to +2.5% with the full set).",
			threePiece(ItemID.INQUISITORS_HELM, ItemID.INQUISITORS_BODY, ItemID.INQUISITORS_SKIRT)));
		...
		sets.add(new ItemSet("Obsidian armour",
			"+10% accuracy and damage with obsidian melee weapons (sword/mace/maul). Stacks with the berserker necklace's +20% damage bonus.",
			obsidianSlots));
		...
		sets.add(fourPiece("Blue moon armour (Frostweaver)",
			"After casting a bind spell, the blue moon spear has a 20% chance for its next melee attack to ignore attack delay.",
		...
		sets.add(new ItemSet("Virtus robes",
			"+2% magic damage per piece (6% full set). While using Ancient Magicks, +5% magic damage per piece (15% full set).",
			threePiece(ItemID.VIRTUS_MASK, ItemID.VIRTUS_TOP, ItemID.VIRTUS_LEGS)));

		Map<EquipmentInventorySlot, int[]> crystalSlots = new EnumMap<>(EquipmentInventorySlot.class);
		crystalSlots.put(EquipmentInventorySlot.HEAD, new int[]{ItemID.GAUNTLET_HELMET_T1});
		crystalSlots.put(EquipmentInventorySlot.BODY, new int[]{ItemID.GAUNTLET_CHESTPLATE_T1});
		crystalSlots.put(EquipmentInventorySlot.LEGS, new int[]{ItemID.GAUNTLET_PLATELEGS_T1});
		crystalSlots.put(EquipmentInventorySlot.WEAPON, new int[]{ItemID.ROVING_CRYSTAL_BOW_NEW});
		sets.add(new ItemSet("Crystal armour",
			"+30% accuracy and +15% damage with the crystal bow or bow of Faerdhinen (helm 5%/2.5%, body 15%/7.5%, legs 10%/5%).",
			crystalSlots));
		...
		singles.put(ItemID.EMBERLIGHT, new SingleItemEffect("Emberlight", "+70% accuracy and damage against demons."));
		singles.put(ItemID.JEWL_BESERKER_NECKLACE, new SingleItemEffect("Berserker necklace", "+20% damage with obsidian weapons. Stacks with the obsidian armour set's +10% bonus."));
		...
		singles.put(ItemID.DRAGONHUNTER_XBOW, new SingleItemEffect("Dragon hunter crossbow", "+30% accuracy and +25% damage against draconic creatures."));
		singles.put(ItemID.DRAGONHUNTER_LANCE, new SingleItemEffect("Dragon hunter lance", "+20% accuracy and damage against draconic creatures."));
		singles.put(ItemID.DRAGONHUNTER_WAND, new SingleItemEffect("Dragon hunter wand", "+75% accuracy and +40% damage against draconic creatures."));
		singles.put(ItemID.WILD_CAVE_BOW_CHARGED, new SingleItemEffect("Craw's bow", "While charged with revenant ether: +50% ranged accuracy and damage against any NPC in the Wilderness (consumes ether per attack)."));
		singles.put(ItemID.WILD_CAVE_WEBWEAVER_CHARGED, new SingleItemEffect("Webweaver bow", "While charged with revenant ether: +50% ranged accuracy and damage against any NPC in the Wilderness (consumes ether per attack)."));
		singles.put(ItemID.WILD_CAVE_SCEPTRE_CHARGED, new SingleItemEffect("Thammaron's sceptre", "While charged with revenant ether: +50% magic accuracy and damage against any NPC in the Wilderness (consumes ether per attack)."));
		singles.put(ItemID.WILD_CAVE_ACCURSED_CHARGED, new SingleItemEffect("Accursed sceptre", "While charged with revenant ether: +50% magic accuracy and damage against any NPC in the Wilderness (consumes ether per attack)."));
		singles.put(ItemID.WILD_CAVE_CHAINMACE_CHARGED, new SingleItemEffect("Viggora's chainmace", "While charged with revenant ether: +50% melee accuracy and damage against any NPC in the Wilderness (consumes ether per attack)."));
		singles.put(ItemID.WILD_CAVE_URSINE_CHARGED, new SingleItemEffect("Ursine chainmace", "While charged with revenant ether: +50% melee accuracy and damage against any NPC in the Wilderness (consumes ether per attack)."));
		...
		singles.put(ItemID.TOXIC_SOTD_CHARGED, new SingleItemEffect("Toxic staff of the dead", "12.5% chance to negate the rune cost of a combat spell cast with it."));
		...
		singles.put(ItemID.SANGUINESTI_STAFF, new SingleItemEffect("Sanguinesti staff", "1-in-5 chance to deal 8 extra damage and heal the wielder for half that amount."));
		singles.put(ItemID.SILVERLIGHT, new SingleItemEffect("Silverlight", "+60% max hit against demons."));
		singles.put(ItemID.DARKLIGHT, new SingleItemEffect("Darklight", "+60% accuracy and damage against demons."));
		singles.put(ItemID.ARCLIGHT, new SingleItemEffect("Arclight", "+70% accuracy and damage against demons."));
		singles.put(ItemID.SCORCHING_BOW, new SingleItemEffect("Scorching bow", "+30% accuracy and damage against demons."));
		...
		singles.put(ItemID.KERIS_PARTISAN, new SingleItemEffect("Keris partisan", "+33% damage against kalphites and scarab creatures. 1/51 chance to deal triple damage."));
		singles.put(ItemID.KERIS_PARTISAN_BREACH, new SingleItemEffect("Keris partisan of breaching", "+33% accuracy and damage against kalphites and scarab creatures. 1/51 chance to deal triple damage."));
		singles.put(ItemID.KERIS_PARTISAN_SUN, new SingleItemEffect("Keris partisan of the sun", "+33% damage against kalphites and scarab creatures. In the Tombs of Amascut, +25% accuracy against targets below 25% health, and killing one heals 12 HP for 5 Prayer."));
		...
		singles.put(ItemID.OSMUMTENS_FANG, new SingleItemEffect("Osmumten's fang", "Rolls accuracy twice and uses the higher roll. Hits deal 15%-85% of max hit, with no zero-damage roll."));
		singles.put(ItemID.TUMEKENS_SHADOW, new SingleItemEffect("Tumeken's shadow", "Triples the magic attack and damage bonuses from worn equipment (quadruples inside the Tombs of Amascut)."));
		singles.put(ItemID.HALLOWFELL, new SingleItemEffect("Hallowfell", "Cleaves into up to 2 additional unique targets within the 5 tiles in front of and beside you, for up to 50% of your max hit each."));
		singles.put(ItemID.ZURIELS_STAFF, new SingleItemEffect("Zuriel's staff", "Boosts Ancient Magicks: ice spells +10% accuracy, blood spells heal 50% more, smoke spells double poison damage, shadow spells double stat drain."));
		...
		singles.put(ItemID.RAT_BONE_STAFF, new SingleItemEffect("Bone staff", "+10 to max hit. Only effective against rats."));
		singles.put(ItemID.BARRONITE_MACE, new SingleItemEffect("Barronite mace", "+15% accuracy and damage against golems."));
		singles.put(ItemID.GRANITE_HAMMER, new SingleItemEffect("Granite hammer", "+30% accuracy and damage against golems."));

		// shields
		singles.put(ItemID.ELYSIAN, new SingleItemEffect("Elysian spirit shield", "70% chance to reduce incoming damage by 25%."));
		singles.put(ItemID.SPECTRAL, new SingleItemEffect("Spectral spirit shield", "Reduces the effectiveness of prayer-draining attacks by 50%."));
		singles.put(ItemID.DINHS_BULWARK, new SingleItemEffect("Dinh's bulwark", "Block special reduces incoming damage by 20%. Pummel special increases max hit based on your defensive bonuses."));
		...
		singles.put(ItemID.DRAGONFIRE_SHIELD, new SingleItemEffect("Dragonfire shield", "Protects against dragonfire and wyverns' icy breath."));
		singles.put(ItemID.WYVERN_SHIELD, new SingleItemEffect("Ancient wyvern shield", "Full immunity to wyverns' freezing breath."));
		singles.put(ItemID.DRAGONFIRE_WARD, new SingleItemEffect("Dragonfire ward", "Protects against dragonfire and wyverns' icy breath."));
		singles.put(ItemID.BROODOO_POISONSHIELD_10, new SingleItemEffect("Broodoo shield (charged)", "Chance to lower the opponent's Attack, Defence, or Strength level (fixed per shield colour), draining 1 charge."));

		// headgear
		singles.put(ItemID.HARMLESS_BLACK_MASK_10, new SingleItemEffect("Black mask", "+16.67% melee accuracy and damage against your assigned Slayer task."));
		singles.put(ItemID.SLAYER_HELM, new SingleItemEffect("Slayer helmet", "+16.67% melee accuracy and damage against your assigned Slayer task."));
		singles.put(ItemID.SLAYER_HELM_I, new SingleItemEffect("Slayer helmet (i)", "+16.67% accuracy and damage in every combat style against your assigned Slayer task."));
		singles.put(ItemID.SERPENTINE_HELM, new SingleItemEffect("Serpentine helm", "While charged with venom: chance to envenom NPCs on hit - 100% with a venom weapon, 50% with a poison weapon, 16.7% otherwise."));

		// handwear
		singles.put(ItemID.GAUNTLETS_OF_CHAOS, new SingleItemEffect("Chaos gauntlets", "+3 maximum hit on Bolt spells."));
		...
		// amulets
		singles.put(ItemID.NECKLACE_OF_FAITH, new SingleItemEffect("Necklace of faith", "When hit below 20% Hitpoints, restores 25% of Prayer points, then breaks."));
		...
		singles.put(ItemID.AMULET_OF_GLORY, new SingleItemEffect("Amulet of glory", "While charged: increases gem-finding chance while Mining and speeds up gem rock respawns."));
		singles.put(ItemID.WILD_CAVE_AMULET, new SingleItemEffect("Amulet of avarice", "Reveals revenants' drops before looting. +20% damage and accuracy against revenants. Permanently skulls the wearer."));
		...
		singles.put(ItemID.RING_OF_RECOIL, new SingleItemEffect("Ring of recoil", "Returns 10% of damage taken from an NPC back at the attacker."));
