...
final class SalveAmulet
{
	...
	static EffectLine describe(int rawItemId)
	{
		...
		{
			return new EffectLine("Salve amulet (ei)", "+20% accuracy and damage in every combat style against the undead.");
		}

		if (contains(IMBUED, rawItemId))
		{
			return new EffectLine("Salve amulet (i)", "+16.67% melee/ranged accuracy and damage, +15% magic accuracy and damage against the undead.");
		}

		if (rawItemId == ENCHANTED)
		{
			return new EffectLine("Salve amulet (e)", "+20% melee accuracy and damage against the undead.");
		}

		return new EffectLine("Salve amulet", "+16.67% melee accuracy and damage against the undead.");
	}
